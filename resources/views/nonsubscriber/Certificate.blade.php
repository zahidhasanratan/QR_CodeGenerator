                            ...
                            <div class="form-row">
                                    ...
                                    @if((isset($subscription_fees_submit->userid) and (Auth::user()->id == $subscription_fees_submit->userid)))
                                        <div class="col-md-12 mb-12 mb-6">
                                            <div class="alert alert-info alert-info-custom">
                                                <a href="{{route('non_Subscription_certificate_details',$subscription_paid_subscriber->subscriptionid)}}" class="btn btn-xs danger pull-right btn-custom-payment">View</a>
                                ...
                                @foreach($seminar_paid_subscriber as $seminar_paid_subscriber)
                                    @if(date('Y-m-d') > (\App\Models\Seminar::where(['id' => $seminar_paid_subscriber->seminar_id])->pluck('eventdate')->first()))
                                        ...
                                        <div class="col-md-12 mb-12 mb-6">
                                            <div class="alert alert-info alert-info-custom">
                                                <a href="{{route('nonSeminar_certificate_details',$seminar_paid_subscriber->seminar_id)}}" class="btn btn-xs danger pull-right btn-custom-payment">View</a>
                                                {{ \App\Models\Seminar::where(['id' => $seminar_paid_subscriber->seminar_id])->pluck('title')->first() }} {{ \App\Models\Seminar::where(['id' => $seminar_paid_subscriber->seminar_id])->pluck('title')->first() }} <strong style="color: #8e24aa; float: right; padding-right: 25px">   Certificate </strong>
                                            </div>
                                        </div>
                                    @endif
                                @endforeach
